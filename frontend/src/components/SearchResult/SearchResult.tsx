...
export default function SearchResult({ search }: { search: Search }) {
  ...
  return (
    <>
      <motion.article
        className='lg:flex lg:space-x-7 lg:justify-center lg:mt-2 mt-5 mb-5 '
        initial={{ opacity: 0 }}
        animate={{
          opacity: !search ? 0 : 1,
        }}
        ...
        <div className='bg-white border-2 border-[#7FCEFF] rounded-lg drop-shadow-xl lg:border-[3px] w-[363px]  lg:w-[935px] lg:h-[195px]'>
          <div className='flex lg:p-8 justify-between'>
            <div className='flex gap-4 items-center '>
              <AvatarProfile
                avatar={search.avatar as string}
                fullName={search.fullName as string}
              />
              ...
              <div className='flex lg:flex-col gap-2 lg:gap-0 mt-6 space-y-3'>
                <div className='flex items-center '>
                  ...
                  <p className='ml-2 text-sm font-normal text-gray-500'>
                    <strong className='text-black'>${search.rating || "n/a"}</strong> (30)
                  </p>
                </div>
                <p className='lg:font-normal lg:text-sm lg:text-end lg:mb-2 mb-3 text-xs mt-3 lg:mt-0'>
                  <strong> ${search.price || "n/a"}</strong> / consulta
                </p>
                <button
                  type='button'
                  ...
                  onClick={() => route.push(`/buscar/${search.id}`)}
                >
                  Ver perfil
                </button>
              </div>
