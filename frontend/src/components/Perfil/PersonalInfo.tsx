...
export default function PersonalInfo({user}:PropsType){
  const pStyle = "w-[310px] font-primary-roboto pl-3 flex border-b-2 border-[#AAAAAA] h-[32px] text-[16px]";
  return(
    <>
      <div className="w-fit shadow-md items-center m-auto p-10 pb-0 rounded-xl border-4 border-[#7FCEFF]">
        ...
        <div className="flex items-center">
          ...
          <h1 className="ml-5 w-fit m-auto font-bold font-primary-roboto text-[25px]">{user.firstName} {user.surName}</h1> 
          <span className="flex justify-center m-auto font-primary-roboto">
            <MdCake size={25} color="#2E2E2E" className="items-center m-2"/>
            <p className="m-auto text-center">{user.birthday?.toLocaleDateString().split("T")[0]}</p>
          </span>
        </div>
        ...
        <div className="flex mt-8 space-x-40">
          ...
          <div className="space-y-4">
            ...
            <span className="flex items-center">
              <MdWork size={25} color="#2E2E2E" /> <p className={pStyle}>{user.rolContent?.occupation}</p>
            </span> 
          </div>
