  ...
  return(
    <div className="w-[356px] text-left flex flex-col m-auto px-3 space-y-4">
      ...
      <label  className="flex px-1 flex-col h-[87px]">Contraseña (*)
        <input name="password" type="password" value={confirm.password} className={ inputStyle } placeholder="Crear contraseña" {...register("password", { required: true, pattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{1,12}$/ })} onChange={(e)=>{setConfirm({...confirm,password:e.target.value});}}></input>
      </label>
      {errors.name && <span>Este campo es requerido</span>}
