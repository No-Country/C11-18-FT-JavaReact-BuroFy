    ...
    <>
      <div className="w-[356px] text-left flex justify-center flex-col m-auto px-3 lg:grid lg:grid-cols-2 lg:w-full lg:mx-5">
        ...
        <label className={labelStyle}>Seleccionar tu país (*)
          <select className={inputStyle} {...register("country", { required: true })} onChange={handleCountryChange}>
            <option disabled selected>Selecciona tu país</option>
            {countries?.map(a => <option value={a.countryName} key={a.countryCode}>{a.countryName}</option>)}
          </select>
        ...
        <label className={labelStyle}>Seleccionar tu ciudad (*)
          <select className={inputStyle} {...register("city", { required: true })}>
            ...
            {provinces?.map(c => <option key={c.name}>{c.name}</option>)}
          </select>
          {errors.city && <span>Este campo es requerido</span>}
        </label>
        
        <label className={labelStyle}>Número de telefono (*)
          <input placeholder="Ingrese su numero" type="number" className={inputStyle} name="phone" {...register("phone", { required: true })}></input>
          {errors.phone && <span>Este campo es requerido</span>}
        </label>
        <label className={labelStyle}>Identificación (*)
          <input placeholder="Ingrese identificación" className={inputStyle} {...register("documentNumber", { required: true })}></input>
          {errors.documentNumber && <span>Este campo es requerido</span>}        
        </label>
        
        <label className={labelStyle}>Número de matricula
          <input placeholder="Sin puntos ni espacios" className={inputStyle} {...register("license", { required: false })}></input>
        </label>
